...
const Secondary = ({ children, isHashLink }) => {
  ...
  return (
    <div className="relative inline-block pr-20 bg-white bg-opacity-0 hover:bg-opacity-10 mb-1 overflow-hidden transition duration-200 group">
      <span className="inline-block w-auto h-auto px-6 md:px-8 py-3 md:py-4 text-lg text-white">
        {children}
      </span>
      <span
        className="absolute top-0 right-0 w-12 group-hover:w-20 bg-gray-800 bg-opacity-0 group-hover:bg-opacity-100 transition-all duration-300 transform origin-right"
